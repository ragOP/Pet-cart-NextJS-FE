    ...
    <div className="w-full md:px-4 pt-3 px-[4%] lg:px-[4%]">
      ...
      >
        {data?.map((coupon, index) => (
          <CarouselItem key={index} className="flex flex-col max-w-fit items-center">
            <button
              type="button"
              aria-label={`Activate coupon ${index + 1}`}
              className="relative w-full h-40 md:h-56 outline-none focus:outline-none bg-transparent rounded-2xl transition-all duration-200 cursor-pointer overflow-hidden"
              onClick={() => {
            ...
            /> */}
              <CustomImage
                src="/coupon-bg.svg"
                alt={`Coupon ${index + 1}`}
                className="absolute inset-0 top-1/2 -translate-y-1/2 w-full h-[180px] object-contain rounded-2xl"
              />
              ...
              <div className="py-1 pl-2 pr-4 relative z-10 flex w-full h-full">
                ...
                <div className="ml-5 flex flex-col items-center justify-evenly py-8 text-[#684A00] font-gotham-rounded font-bold">
                  <span className="text-[10px] md:text-xs font-medium text-[#323232]">VOUCHER</span>
                  <div className="flex flex-col items-center tracking-0 leading-[100%] space-y-1 font-holtwood font-[400]">
                    <span className="text-[16px] md:text-[22px] px-2">{coupon?.discountValue}{coupon?.discountType !== "fixed" ? "%" : "Rs"}</span>
