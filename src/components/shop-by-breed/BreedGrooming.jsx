...
const BreedGrooming = ({ breed }) => {
    // Fallback if grooming data is not provided
    if (!breed?.grooming) {
        return null;
    }

    const { grooming } = breed;
    const groomingTips = grooming.tips || [];

    return (
        <div className="relative mt-8">
            {/* Title and Subtitle Section */}
            <div className="px-[5%] mb-4 lg:mb-0">
                <PawsTitle title={grooming.title || "GROOMING"} classNameTitle={"text-[20px] sm:text-[24px] md:text-[28px] lg:text-[32px] pt-2 "} imageProps={{
                    height: 60,
                    width: 60,
            ...
            {/* Original Flex Row - Text and Image (swapped) */}
            <div className="px-[2%] md:px-[5%] mb-8">
                    ...
                    <div className='w-full lg:w-[60%] flex flex-col justify-center'>
                        <div className='w-full lg:w-[80%] flex flex-col justify-center px-4 lg:px-0'>
                            <p 
                                ...
                                dangerouslySetInnerHTML={{ __html: grooming.mainDescription }}
                            />
                        </div>
                    </div>

                    {/* Right Side - Image */}
                    <div className="w-full lg:w-[40%] relative flex justify-center lg:justify-end h-40 sm:h-48 lg:h-full">
                        <CustomImage
                            src={grooming.image || '/gm-6.png'}
                            alt={`${breed.name} - Grooming`}
            ...
            {/* Grooming Tips Cards - Similar to BreedDietTips */}
