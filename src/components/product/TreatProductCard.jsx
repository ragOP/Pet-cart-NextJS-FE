...
const TreatProductCard = ({ item }) => {
    ...
    const handleAddtoCart = (e) => {
      e.stopPropagation();
      addToCart({ productId: item._id, variantId: null, quantity: 1 });
    };
  ...
  return (
    <div className="group flex flex-col items-center rounded-xl p-2 transition-all duration-200 hover:scale-105 hover:ring-2 hover:ring-[#004E6A] focus-within:scale-105 cursor-pointer bg-transparent">
      ...
      <button
        className="w-30 mt-2 border-2 border-[#004E6A] text-[#004E6A] py-1 rounded-md font-semibold md:w-35 md:h-10 hover:bg-[#004E6A] hover:text-white focus:bg-[#004E6A] focus:text-white transition-colors duration-300 outline-none cursor-pointer disabled:bg-gray-400 disabled:cursor-not-allowed "
